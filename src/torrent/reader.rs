...
use super::bencode::{parse, Bencode};
...
pub fn read_torrent(path: &str) -> Result<TorrentMetadata, TorrentError> {
    ...
    let announce = match &announce.value {
        ...
        _ => {
            return Err(TorrentError::Parse(
                "announce must be a string".into(),
            ))
        }
    };
    ...
    _ => {
        return Err(TorrentError::Parse(
            "info must be a dictionary".into(),
        ))
    }
};
...
let piece_length = info_dict
    .get("piece length")
    .ok_or_else(|| TorrentError::Parse(
        "missing piece length".into(),
    ))?;
...
let total_length = match &total_length.value {
    ...
    _ => {
        return Err(TorrentError::Parse(
            "length must be an integer".into(),
        ))
    }
};
...
if pieces_bytes.len() % 20 != 0 {
    return Err(TorrentError::Parse(
        "pieces field is malformed".into(),
    ));
}
